    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-xs text-muted-foreground">
          Mapa visual dos {devices.length} dispositivos detectados na rede — dados em tempo real
        </p>
      </div>
        ...
        <div className="xl:col-span-2 rounded-lg border border-border bg-card card-shadow overflow-hidden">
          {devices.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <p className="text-sm text-muted-foreground">Nenhum dispositivo detectado ainda.</p>
              <p className="text-xs text-muted-foreground mt-1">Verifique se o agente está rodando.</p>
            </div>
          ) : (
            <svg viewBox="0 0 800 600" className="w-full h-auto" style={{ minHeight: 400 }}>
              ...
              {/* Gateway center node */}
              {gateway && (
                <g className="cursor-pointer" onClick={() => setSelectedIp(gateway.ip)}>
                  ...
                  <text x={400} y={212} textAnchor="middle" fill="hsl(220, 10%, 40%)" fontSize={7}>
                    {(gateway.hostname || "Gateway").substring(0, 20)}
                  </text>
                </g>
              )}
                ...
                return (
                  <g key={device.ip} className="cursor-pointer" onClick={() => setSelectedIp(device.ip)}>
                    ...
                    <text x={x} y={y + 3} textAnchor="middle" fill="hsl(220, 15%, 80%)" fontSize={8} fontFamily="JetBrains Mono">
                      {(device.hostname || device.mac).substring(0, 10)}
                    </text>

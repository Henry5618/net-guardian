...
import { getNetworkDevices, type NetworkDevice } from "@/lib/mock-data";
import { cn } from "@/lib/utils";
import { Monitor, Router, Server, Printer, Camera, Wifi, Smartphone, Cpu, Tv, HardDrive, Laptop } from "lucide-react";
...
const deviceIconMap: Record<string, typeof Monitor> = {
  ...
  "laptop-dev01": Laptop,
  "server-db": Server,
  "nas-storage": HardDrive,
  "printer-office": Printer,
  "cam-lobby": Camera,
  "ap-wifi-01": Wifi,
  "mobile-user01": Smartphone,
  "iot-sensor-temp": Cpu,
  "smart-tv": Tv,
  "guest-laptop": Laptop,
};
...
export default function TopologyPage() {
  ...
  const gateway = devices.find((d) => d.hostname === "gateway-router");
  const others = devices.filter((d) => d.hostname !== "gateway-router");

  // Arrange devices in a circle around the gateway
        ...
        <div className="xl:col-span-2 rounded-lg border border-border bg-card card-shadow overflow-hidden">
          <svg viewBox="0 0 800 600" className="w-full h-auto" style={{ minHeight: 400 }}>
            ...
            {gateway && (
              ...
              >
                <circle cx={400} cy={300} r={32} fill="hsl(210, 100%, 50%)" fillOpacity={0.15} stroke="hsl(210, 100%, 50%)" strokeWidth={2} />
                <circle cx={400} cy={300} r={24} fill="hsl(220, 15%, 10%)" stroke="hsl(210, 100%, 50%)" strokeWidth={1.5} />
                <text x={400} y={304} textAnchor="middle" fill="hsl(210, 100%, 60%)" fontSize={10} fontWeight="bold">GW</text>
                <text x={400} y={350} textAnchor="middle" fill="hsl(220, 10%, 50%)" fontSize={9}>{gateway.ip}</text>
              </g>
            )}
            ...
            {positions.map(({ device, x, y }) => {
              ...
              return (
                <g key={device.ip} className="cursor-pointer" onClick={() => setSelected(device)}>
                  <circle
          ...
          {selected ? (
            <div className="space-y-3">
              {[
                ...
                { label: "Fabricante", value: selected.vendor },
                { label: "Status", value: selected.status === "online" ? "🟢 Online" : "🔴 Offline" },
                { label: "Último Visto", value: selected.lastSeen },
                { label: "Enviado", value: `${(selected.traffic.sent / 1024).toFixed(1)} KB` },
                { label: "Recebido", value: `${(selected.traffic.received / 1024).toFixed(1)} KB` },
                { label: "Portas Abertas", value: selected.openPorts.join(", ") || "Nenhuma" },
              ].map((item) => (
